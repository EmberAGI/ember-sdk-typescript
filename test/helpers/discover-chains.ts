 ...
 */
export const discoverChains = async (): Promise<Array<{ chainId: number; rpcUrl: string }>> => {
...
export const validateRequiredChains = (
  ...
  for (const requiredChainId of requiredChainIds) {
    if (!availableChainIds.includes(requiredChainId)) {
      throw new Error(`Required chain ID ${requiredChainId} is not available. Available chains: ${availableChainIds.join(", ")}`);
    }
  }
};
...
export const getRpcUrlForChain = (
  ...
  if (!chain) {
    throw new Error(`Chain ID ${chainId} not found in available chains: ${availableChains.map(c => c.chainId).join(", ")}`);
  }
  return chain.rpcUrl;
}; 