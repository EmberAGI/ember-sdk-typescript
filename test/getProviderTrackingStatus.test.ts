...
import {
  MockEmberClient,
  StatusVariantMockEmberClient,
  ErrorMockEmberClient,
} from "./mocks/mock-clients";
import {
  assertValidTrackingStatus,
  assertErrorThrown,
} from "./helpers/shared-assertions";
...
describe("Integration tests for getProviderTrackingStatus", function () {
  ...
  describe("Live Client Tests", function () {
    ...
    it("should successfully return tracking status for a valid transaction", async () => {
      ...
      expect(response.trackingStatus!.requestId).to.equal(
        TEST_CONSTANTS.SUCCESS_REQUEST_ID,
      );
      expect(response.trackingStatus!.transactionId).to.equal(
        TEST_CONSTANTS.SUCCESS_TRANSACTION_ID,
      );
      expect(response.trackingStatus!.status).to.equal(
        ProviderStatus.PROVIDER_STATUS_SUCCESS,
      );
    });
  ...
  describe("Mock Client Tests", function () {
    ...
    describe("Success Variant Tests", function () {
      it("should handle PARTIAL_SUCCESS status", async () => {
        const variantClient = new StatusVariantMockEmberClient(
          ProviderStatus.PROVIDER_STATUS_PARTIAL_SUCCESS,
        );
      ...
      it("should handle NEEDS_GAS status", async () => {
        const variantClient = new StatusVariantMockEmberClient(
          ProviderStatus.PROVIDER_STATUS_NEEDS_GAS,
        );
        const response = await variantClient.getProviderTrackingStatus({
          requestId: TEST_CONSTANTS.SUCCESS_REQUEST_ID,
          transactionId: TEST_CONSTANTS.SUCCESS_TRANSACTION_ID,
      ...
      it("should handle ONGOING status", async () => {
        const variantClient = new StatusVariantMockEmberClient(
          ProviderStatus.PROVIDER_STATUS_ONGOING,
        );
        const response = await variantClient.getProviderTrackingStatus({
          requestId: TEST_CONSTANTS.SUCCESS_REQUEST_ID,
          transactionId: TEST_CONSTANTS.SUCCESS_TRANSACTION_ID,
